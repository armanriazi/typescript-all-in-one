...
const returnIdValue = objEmit.pipe(
    map((value: INestedObj) => {
      ...
      // Use the catchError operator to catch and handle any errors that occur during the mapping operation.
  catchError((error: unknown) => {
    // Log the error to the console.
    console.log(`stream caught : ${error}`);
    // Return an observable that emits a single null value to indicate that an error occurred.
    return of(null);
  })
  );
